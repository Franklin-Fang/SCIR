...
def check_missing(data,inputs,task):
    missing_lora_path = './model/Qwen3-4B/missing'
    ...
    outputs = llm.generate(
        texts,
        sampling_params,
        lora_request=LoRARequest("missing", 2, missing_lora_path)
    )
    results = []
...
def check(data,task):
    ...
    data = check_missing(data,inputs,task)
    return data